...
export const TransactionHistory: FC<TransactionHistoryProps> = ({ transactions }) => {
  const getStatusBadge = (status: string) => {
    ...
    return (
      <span class={`badge ${statusClasses[status as keyof typeof statusClasses] || 'badge-neutral'}`}>
    ...
    <div class="max-w-6xl mx-auto">
      ...
      {transactions.length === 0 ? (
        <div class="text-center py-8">
          <p class="text-gray-500">No transactions yet. Start your first withdrawal!</p>
          <button 
            ...
            <tbody>
              {transactions.map(tx => (
                <tr class="hover">
                  ...
                  <td>
                    <div class="font-mono text-xs max-w-xs truncate" title={tx.address}>
                      {tx.address}
                    </div>
                  </td>
...
export const TransactionDetails: FC<{ transaction: Transaction }> = ({ transaction }) => {
  return (
    <div class="py-4">
      <div class="grid grid-cols-2 gap-4">
        ...
        <div>
          ...
          <div class="mt-1">
            <span class={`badge ${
              ...
              transaction.status === 'processing' ? 'badge-info' : 'badge-warning'
            }`}>
        ...
        <div>
          ...
          <div>{transaction.amount} {transaction.coin}</div>
        </div>
        <div>
          <strong>Fee:</strong>
          <div>{transaction.fee || 0} {transaction.coin}</div>
        </div>
        <div class="col-span-2">
          <strong>Withdrawal Address:</strong>
          <div class="font-mono text-sm break-all bg-base-200 p-2 rounded mt-1">
            {transaction.address}
          </div>
        </div>
        {transaction.error && (
          <div class="col-span-2">
            <strong>Error:</strong>
            <div class="text-error bg-error/10 p-2 rounded mt-1">
              {transaction.error}
            </div>
          </div>
        )}
