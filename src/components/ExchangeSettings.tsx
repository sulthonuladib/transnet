  ...
  return (
    <div class='space-y-6'>
              ...
              <div class='card-body'>
                  ...
                  <div class='flex items-center space-x-3'>
                    ...
                    <div>
                      ...
                      <div class='flex items-center space-x-2'>
                        <div
                          class={`badge ${isConfigured ? 'badge-success' : 'badge-warning'}`}
                        >
                        ...
                        {config && (
                          <>
                            <div
                              class={`badge ${config.isActive ? 'badge-info' : 'badge-neutral'}`}
                            >
                              {config.isActive ? 'Active' : 'Inactive'}
                            </div>
                            {config.isActive && (
                              <div
                                class={`badge ${config.isValid ? 'badge-success' : 'badge-warning'}`}
                              >
                                {config.isValid
                                  ? '✓ Valid'
                                  : '⚠ Not Validated'}
                ...
                {isConfigured && (
                  <div class='text-base-content/70 mt-4 text-sm'>
                    <p>
                      <strong>API Key:</strong> {config.apiKey?.slice(0, 8)}...
                    ...
                    {config.lastValidationAt && (
                      <p>
                        ...
                        {new Date(config.lastValidationAt).toLocaleString()}
                      </p>
                    )}
      ...
      <dialog id='modal' class='modal'>
        <div class='modal-box'>
          <div id='modal-content'>{/* Content will be loaded here */}</div>
        </div>
  ...
  return (
    <div>
      <h3 class='mb-4 text-lg font-bold'>
        {isEdit
          ? `Edit ${exchangeName} Configuration`
          : `Configure ${exchangeName}`}
      </h3>
